...
const GradientBackground = () => (
    <>
        ...
        <svg width="100%" height="100%" viewBox="0 0 1000 800" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full bg-[#fdfdfd]">
            <defs>
            ...
            <g style={{ animation: 'float1 20s ease-in-out infinite' }}>
                {/* Soft Peach/Orange on the mid-right */}
                <ellipse cx="850" cy="450" rx="400" ry="350" fill="#ffb48f" filter="url(#blurXl)" opacity="0.6"/>
                {/* Light Yellow on the top right */}
                <ellipse cx="700" cy="100" rx="300" ry="250" fill="#fff5cc" filter="url(#blurLg)" opacity="0.8"/>
            </g>
            <g style={{ animation: 'float2 25s ease-in-out infinite' }}>
                {/* Grayish blue on the bottom left */}
                <ellipse cx="150" cy="650" rx="350" ry="300" fill="#8b9ba8" filter="url(#blurXl)" opacity="0.65"/>
                {/* Very light gray/blue subtle blob in the center left */}
...
const DiscordIcon = (props: React.SVGProps<SVGSVGElement>) => ( <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 127.14 96.36" className="w-5 h-5"> <path fill="currentColor" d="M107.7,8.07A105.15,105.15,0,0,0,81.47,0a72.06,72.06,0,0,0-3.36,6.83A97.68,97.68,0,0,0,49,6.83,72.37,72.37,0,0,0,45.64,0,105.89,105.89,0,0,0,19.39,8.09C2.79,32.65-1.71,56.6.54,80.21h0A105.73,105.73,0,0,0,32.71,96.36,77.7,77.7,0,0,0,39.6,85.25a68.42,68.42,0,0,1-10.85-5.18c.91-.66,1.8-1.34,2.66-2a75.57,75.57,0,0,0,64.32,0c.87.71,1.76,1.39,2.66,2a68.68,68.68,0,0,1-10.87,5.19,77,77,0,0,0,6.89,11.1,105.25,105.25,0,0,0,32.19-16.14h0c2.64-27.38-4.51-51.11-19.32-72.15ZM42.68,68.31c-5.28,0-9.61-4.86-9.61-10.82s4.24-10.82,9.61-10.82c5.42,0,9.66,4.89,9.61,10.82,0,5.96-4.24,10.82-9.61,10.82Zm41.74,0c-5.28,0-9.61-4.86-9.61-10.82s4.24-10.82,9.61-10.82c5.42,0,9.66,4.89,9.61,10.82,0,5.96-4.24,10.82-9.61,10.82Z"/> </svg> );

const modalSteps = [
    { message: "Authenticating you...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize" }: AuthComponentProps) => {
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (modalStatus !== 'closed' || authStep !== 'password') return; // Modified from confirmPassword to password to make it act like Login
    ...
    setTimeout(() => {
        ...
        // Let it show success for a bit before redirecting
        setTimeout(() => {
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
        ...
        if (isPasswordValid) {
            // Act as final submit directly from password for login
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col">
        ...
        <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-card", "relative overflow-hidden")}>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-10 w-full max-w-[700px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-6">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-['Playfair_Display'] font-medium text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">Welcome</p></div></BlurFade>
                        <BlurFade delay={0.25 * 2}><p className="text-base font-medium text-muted-foreground">Continue with</p></BlurFade>
                        <BlurFade delay={0.25 * 3} className="w-full">
                            <div className="flex flex-row items-center justify-center gap-6 w-full">
                                <GlassButton className="shrink-0" contentClassName="flex items-center justify-center gap-3 px-8 py-4" size="lg">
                                    <GoogleIcon />
                        ...
                        <BlurFade delay={0.25 * 4} className="w-full max-w-[360px]"><div className="flex items-center w-full gap-2 py-4"><hr className="w-full border-border"/><span className="text-xs font-semibold text-muted-foreground">OR</span><hr className="w-full border-border"/></div></BlurFade>
                    </motion.div>}
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-['Playfair_Display'] font-medium text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">Welcome Back</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Enter your password to sign in</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>
                
                <form onSubmit={handleFinalSubmit} className="w-full max-w-[320px] space-y-6">
                     <AnimatePresence>
